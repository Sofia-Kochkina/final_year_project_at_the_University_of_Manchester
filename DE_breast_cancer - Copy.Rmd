...
```
```{r}
normalized_counts <- counts(dds, normalized=TRUE)   #write normalized counts into a file to use as an input for PCA and tSNE in Python
...
```{r}
rld <- rlog(dds, blind=TRUE)                                                 #here can choose rlog() or vst(); note the batch effect
rld_mat <- assay(rld)   
...
```{r}
#vsdata <- vst(dds, blind=TRUE)      #this PCA plot did not produce good clustering based on MD, so sckit-learn plot was used
#plotPCA(vsdata, ntop=20000, PCs = c(1,2))  #def ntop=1000
